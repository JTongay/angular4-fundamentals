...
export class EventService {
    ...
    getEventFromServer(id :number):Observable<IEvent> {
    ...
    saveEventToServer(event): Observable<IEvent> {
      let headers = new Headers({ 'Content-Type': 'application/json' });
      let options = new RequestOptions({headers: headers});

      return this.http.post('/api/events', JSON.stringify(event), options).map((res: Response) => {
        return res.json();
      }).catch(this.handleError);
    }
    ...
    searchSessionsFromServer(searchTerm: string):Observable<any> {
